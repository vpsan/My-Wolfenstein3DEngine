...
/*   Updated: 2021/04/19 16:27:46 by bhatches         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	rcstng_calculate_wallx(t_cube3D *cube)
{
	...
	if (cube->rcstg.side == 1 && cube->rcstg.ray_dir_y < 0)
		cube->rcstg.tex_x = cube->rcstg.tex->width - cube->rcstg.tex_x - 1;
	cube->rcstg.tex_step = (double)(cube->rcstg.tex->height)
		/ cube->rcstg.line_height;
	cube->rcstg.tex_pos = (cube->rcstg.draw_start - cube->map_prmtrs.win_height
			/ 2 + cube->rcstg.line_height / 2) * cube->rcstg.tex_step;
	return (0);
}
